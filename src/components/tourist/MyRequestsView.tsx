      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-[#E8DFC9] shadow-xs">
        <div>
          <span className="text-xs font-bold text-[#D97843] uppercase tracking-wider block mb-1">
            Traveler Inquiries, Live Quotes & Escrow
          </span>
          ...
          <p className="text-xs text-[#6B7A72] mt-0.5">
            Track operator responses, negotiate details, review custom TZS quotes, and pay via M-Pesa / Tigo Pesa.
          </p>
        </div>
      </div>

      {leads.length === 0 ? (
        <div className="bg-white rounded-3xl border border-[#E8DFC9] p-12 text-center max-w-md mx-auto space-y-4">
          <div className="w-14 h-14 rounded-full bg-[#FAF7F2] text-[#6B7A72] flex items-center justify-center mx-auto text-xl border border-[#E8DFC9]">
            <FileText className="w-6 h-6 text-[#284435]" />
          </div>
          <h3 className="font-serif text-lg font-bold text-[#284435]">No Inquiries Sent Yet</h3>
        ...
        <div className="space-y-6">
          {leads.map((lead) => {
            ...
            return (
              ...
              >
                {/* Lead Header */}
                <div className="p-6 border-b border-[#F0EBE0] flex flex-wrap items-center justify-between gap-4 bg-[#FAF7F2]">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-serif text-lg font-bold text-[#284435]">
                        {lead.listingTitle}
                      </span>
                      {lead.status === 'Negotiating' && (
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-[#6B7A72]">
                      <span>Operator: <strong className="text-[#284435]">{lead.providerName}</strong></span>
                      <span>•</span>
                      ...
                      <span>•</span>
                      <span className="flex items-center gap-1">
                        <Users className="w-3.5 h-3.5 text-[#284435]" />
                ...
                <div className="p-6 space-y-4">
                  {lead.message && (
                    <div className="text-xs text-[#4D5E55] bg-[#FAF7F2] p-3 rounded-xl border border-[#E8DFC9]">
                      <strong className="text-[#284435]">Your special note:</strong> "{lead.message}"
                    </div>
                  )}

                  {/* Quote Section if provider sent quote */}
                  {lead.quote && (
                    <div className="rounded-2xl border border-[#D97843]/30 bg-linear-to-b from-[#FFFBF7] to-[#FAF6EE] p-5 space-y-4 shadow-xs">
                      <div className="flex items-center justify-between border-b border-[#E8DFC9] pb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h4 className="font-serif text-base font-bold text-[#284435]">
                        ...
                        <div className="text-right">
                          <span className="text-[10px] uppercase font-bold text-[#8A9790] block">Quote Total</span>
                          <span className="font-bold text-lg text-[#284435]">
                      ...
                      <div className="space-y-2 text-xs">
                        ...
                        {lead.quote.extras.map((ex) => (
                          <div key={ex.id} className="flex justify-between text-[#4D5E55]">
                            <span>+ {ex.title}</span>
                      ...
                      {/* Operator Note */}
                      {lead.quote.note && (
                        <div className="p-3 bg-white rounded-xl border border-[#E8DFC9] text-xs text-[#284435]">
                          <span className="font-bold text-[11px] text-[#D97843] block mb-0.5">Message from Operator:</span>
                          <p className="italic text-[#4D5E55]">"{lead.quote.note}"</p>
                        </div>
                      )}
                      ...
                      {lead.status === 'Negotiating' && (
                        <div className="pt-2 flex flex-wrap items-center gap-3">
                          <button
                            ...
                            className="px-5 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold flex items-center gap-2 transition-all shadow-sm cursor-pointer"
                          >
                            <Lock className="w-4 h-4 text-[#E8B94A]" />
                            <span>{t('accept.pay', 'Accept Quote & Pay via Escrow')}</span>
                          </button>
                          <button
                            onClick={() => declineQuote(lead.id)}
                            className="px-4 py-2.5 rounded-xl bg-white hover:bg-red-50 text-red-700 text-xs font-semibold border border-red-200 transition-colors cursor-pointer"
                          >
                  ...
                  {lead.status === 'Booked' && confirmedBooking && (
                    <div className="bg-[#EAF3EC] p-4 rounded-2xl border border-[#CDE3D4] flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-2.5">
                        <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                        <div>
                          <span className="font-bold text-xs text-[#284435] block">
                            Trip Confirmed & Secured under Escrow (Ref: {confirmedBooking.referenceCode})
                          </span>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-3.5 py-2 rounded-xl bg-[#284435] hover:bg-[#1E332A] text-white text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-xs"
                        >
                        ...
                        {!confirmedBooking.hasReview && (
                          <button
                            ...
                            className="px-3.5 py-2 rounded-xl bg-[#FAF7F2] hover:bg-[#EDE5D5] text-[#284435] text-xs font-bold border border-[#CDE3D4] flex items-center gap-1.5 cursor-pointer shadow-xs"
                          >
                  ...
                  <div className="pt-2 border-t border-[#F0EBE0]">
                    <button
                      ...
                      className="text-xs font-bold text-[#284435] hover:text-[#D97843] flex items-center gap-1.5 cursor-pointer"
                    >
                    ...
                    {isChatOpen && (
                      <div className="mt-3 bg-[#FAF7F2] p-4 rounded-2xl border border-[#E8DFC9] space-y-3">
                        <div className="space-y-2 max-h-48 overflow-y-auto">
                          {(!lead.messages || lead.messages.length === 0) ? (
                            <p className="text-xs text-[#8A9790] italic">
                            ...
                            lead.messages.map((m) => (
                              <div
                                ...
                                className={`p-2.5 rounded-xl text-xs max-w-sm ${
                                  m.sender === 'tourist'
                                    ? 'bg-[#284435] text-white ml-auto'
                                    : 'bg-white text-[#1F2A24] border border-[#E8DFC9] mr-auto'
                                }`}
                              >
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t border-[#E8DFC9]">
                          <input
                            ...
                            className="flex-1 px-3 py-2 text-xs rounded-xl bg-white border border-[#DED5C6] focus:outline-hidden"
                          />
                          <button
                            type="button"
                            onClick={() => handleSendChat(lead.id)}
                            className="px-3 py-2 bg-[#284435] hover:bg-[#1E332A] text-white rounded-xl text-xs font-semibold flex items-center gap-1 cursor-pointer"
                          >
